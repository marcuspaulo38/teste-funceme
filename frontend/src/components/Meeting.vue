<template>
    <div id="meeting">
        ...
        <div v-if="auth" id="status-meeting">
            <button v-if="!status" v-on:click="updateStatus(meeting.id, true)"><i class="icon-accept fa fa-check" aria-hidden="true"></i> OK</button>
            <button v-if="status" v-on:click="updateStatus(meeting.id, false)"><i class="icon-refuse fa fa-times" aria-hidden="true"></i> Recuso</button>
        </div>
