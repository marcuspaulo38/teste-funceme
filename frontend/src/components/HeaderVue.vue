<template>
  <header>
    <div class="container">
      <img src="/static/images/piperun-logo.png" width="120px"/>
      <span v-if="auth">Olá, {{auth.name}}</span> 
      <div class="buttons-header">
        <router-link class="btn btn-login" to="/home">Home</router-link>
        <router-link v-if="!auth" class="btn btn-color btn-login" to="/login">Login</router-link>
        <router-link v-if="auth && this.$route.name != 'Dashboard'" class="btn btn-color btn-login" to="/dashboard">Agenda</router-link>
